...
- `moon info` followed by `git diff --exit-code` leaves the worktree unchanged.
- `moon fmt` followed by `git diff --exit-code` leaves the worktree unchanged.
- `moon run cmd/main` produces the runnable demo output.
